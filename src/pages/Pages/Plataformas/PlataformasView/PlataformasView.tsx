...
import { PlataformasView } from 'interfaces/Plataformas/PlataformasInterface'
import { PlataformasService } from 'services/Plataformas/PlataformasService'
import { corPlataforma } from 'helpers/cores_tema_helpers'

const PlataformasViewPage = () => {
    const { state } = useLocation()
    const { id } = useParams()
    const [record, setRecord] = useState<PlataformasView | null>(state?.source ?? null)
    const { voltarParaRotaAnterior } = useNavegacao()
    const plataformasService = new PlataformasService()

    const loadRecord = async (recordId: string) => {
        try {
            const view = await plataformasService.getViewPlataformas({ id: recordId })
    ...
    const cor = corPlataforma({
        cor: record.cor,
        plataforma_id: record.id,
    })
                        ...
                        <Col xs={12}>
                            <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                                <div className="d-sm-flex align-items-center justify-content-between">
                                    ...
                                    <h4 className="mb-sm-0 ms-3">Visualizar Plataforma</h4>
                                </div>
                                <Breadcrumb pageTitle="" listClassName="mb-sm-0 pt-1 py-2">
                                    <BreadcrumbItem><Link to="/dashboard"><i className="ri-home-5-fill"></i></Link></BreadcrumbItem>
                                    <BreadcrumbItem><Link to="/plataformas">Plataformas</Link></BreadcrumbItem>
                                    <BreadcrumbItem active>Visualizar Plataforma</BreadcrumbItem>
                                </Breadcrumb>
                            </div>
                        </Col>
                    </Row>
                                    ...
                                    <Row className="mt-3">
                                        <Col md={12}>
                                            <div className="hstack gap-2 justify-content-end">
                                                <Link to={`/plataformas/edit/${record.id}`} state={{ source: record }} className="btn btn-primary">
                                                    Editar
                                                </Link>
...
export default PlataformasViewPage
